...
import de.hbrs.ia.code.Personal.ManagePersonalImpl;
...
public class ManageSocialImpl implements ManageSocial{
    ...
    public boolean deleteSocialPerformanceRecord(Integer goalId) {
        ...
        for(SalesMan salesMan : allSalesmen){
            if (salesMan.getPerformanceRecords().contains(goalId)){
                managePersonal.removeSocialPerformanceRecord(goalId,salesMan.getSid());
            }
        }
        socialPerformRecordCollection =  DbConnection.getCollection(COLLECTION_SP_NAME);
